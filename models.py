...
class RENR(nn.Module):
    def __init__(self, n, m, n_xi, l):
        ...
        self.sg = nn.Parameter(torch.randn(1, 1) * 5)  # square root of gamma
...
class pizzicottina(nn.Module):
    ...
    def forward(self, t, ym, d, xim):
        ...
        uf = torch.tensor([u[range(0, 3)], u[range(3, 8)], u[range(8, 11)]])
        y_list = []
        xi_list = []
        start = 0
        stop = 0
        ...
        for j, l in enumerate(self.r):
            ...
            yt, xitemp = self.r[j](t, uf[j], xim[indexx])
            y_list.append(yt)
...
class pizzicottina2(nn.Module):
    ...
    def set_model_param(self):
        ...
        for j, l in enumerate(self.r):
            ...
            Mu = M[indexu, :]
            My = M[:, indexy]
            gamma = torch.sqrt((z / (torch.max(torch.sum(torch.abs(Mu), 1)) * z + 1))
                               / (1 + torch.max(torch.sum(torch.abs(My), 0)) + y[j]))
            self.r[j].gamma = gamma
            self.r[j].set_model_param()

    def forward(self, t, d, T):
        M = self.M
        y = torch.randn(torch.sum(self.n), T + 1, device=device, dtype=dtype)
        xi = torch.randn(12, T + 1, device=device, dtype=dtype)
        u = torch.randn(11, T + 1, device=device, dtype=dtype)
        for k in range(1, T):
            u[:, k] = torch.matmul(M, y[:, k - 1]) + d[:, k]
            y_list = []
            xi_list = []
            start = 0
            stop = 0
            ...
            for j, l in enumerate(self.r):
                ...
                index = range(start, stop)
                indexx = range(startx, stopx)
                yt, xitemp = self.r[j](t, u[index, k - 1], xi[indexx, k - 1])
                y_list.append(yt)
                xi_list.append(xitemp)
            y[:, k] = torch.cat(y_list)
            xi[:, k] = torch.cat(xi_list)
        yf = y
        xf = xi
        return yf, xf
...
class pizzicottinaTank(nn.Module):
    def __init__(self, N, M, n, p, n_xi, l):
        ...
        self.r = nn.ModuleList([RENRG(self.n[j], self.p[j], self.n_xi[j], self.l[j]) for j in range(N)])
        self.y = nn.Parameter(torch.randn(N, device=device, dtype=dtype))
        self.q = nn.Parameter(torch.randn(torch.sum(p), device=device, dtype=dtype))
        self.z = nn.Parameter(torch.randn(1, device=device, dtype=dtype))
        self.gammaw = torch.randn(1, device=device, dtype=dtype)
...
class pizzicottina4(nn.Module):
    def __init__(self, N, M, n, p, n_xi, l):
        ...
        self.b = nn.Parameter(torch.randn(4, device=device, dtype=dtype))
        self.z = nn.Parameter(torch.randn(1, device=device, dtype=dtype))
        self.gammaw = torch.randn(1, device=device, dtype=dtype)
...
class pizzicottina5(nn.Module):
    def __init__(self, N, M, n, p, n_xi, l, T):
        super().__init__()
        self.T = T
        self.p = p
        self.n = n  # nel paper m
        ...
        self.r = nn.ModuleList([RENRG(self.n[j], self.p[j], self.n_xi[j], self.l[j]) for j in range(N)])
        self.y = nn.Parameter(torch.randn(N, device=device, dtype=dtype))
        self.q = nn.Parameter(torch.randn(4, T + 1, device=device, dtype=dtype))
        self.b = nn.Parameter(torch.randn(4, T + 1, device=device, dtype=dtype))
        self.z = nn.Parameter(torch.randn(1, device=device, dtype=dtype))
        self.gammaw = torch.randn(1, device=device, dtype=dtype)
    ...
    def forward(self, t, d):
        M = self.M
        y = torch.zeros(4, self.T + 1, device=device, dtype=dtype)
        ym = torch.zeros(4, self.T + 1, device=device, dtype=dtype)
        xi = torch.randn(24, self.T + 1, device=device, dtype=dtype)
        u = torch.randn(6, self.T + 1, device=device, dtype=dtype)
        for k in range(1, self.T):
            u[:, k] = torch.matmul(M, y[:, k]) + d[:, k]
            y_list = []
            xi_list = []
            start = 0
            stop = 0
            ...
            for j, l in enumerate(self.r):
                ...
                xi_list.append(xitemp)
            y[:, k] = torch.cat(y_list)
            ym[:, k] = y[:, k].clone() * self.q[:, k] + self.b[:, k]
            xi[:, k] = torch.cat(xi_list)
        yf = y
        xf = xi
        return yf, xf, ym
...
class pizzidoublespring(nn.Module):
    def __init__(self, N, M, n, p, n_xi, l):
        super().__init__()
        self.p = p
        self.n = n  # nel paper m
