...
from models import doubleRENg, RENR2, multiRENL2, RENRG, ThreeRENL2GershgorinShur
import numpy as np
import math
...
RENsys = ThreeRENL2GershgorinShur(N, M, n, p, n_xi, l)
...
for epoch in range(epochs):
    ...
    for exp in range(nExp - 1):
        xi = []
        yRENm = torch.zeros(N, t_end + 1, device=device, dtype=dtype)
        ...
        for t in range(1, t_end):
            yRENm[:, t], xi = RENsys(t, yRENm[N - 1, t - 1], u[:, t], xi)
            yREN[t] = yRENm[0, t]
...
for j in range(N):
    xi.append(torch.zeros(RENsys.r[j].n_xi, device=device, dtype=dtype))
yRENm_val = torch.zeros(N, t_end + 1, device=device, dtype=dtype)
yREN_val = torch.zeros(t_end + 1, device=device, dtype=dtype)

u_val = torch.from_numpy(uExp_val[0, 0]).float().to(device)
for t in range(t_end):
    yRENm_val[:, t], xi = RENsys(t, yRENm_val[N - 1, t - 1], u_val[:, t], xi)
    yREN_val[t] = yRENm_val[0, t]
